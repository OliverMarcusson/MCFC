...
const VANILLA_MCDOC_TARBALL_URL: &str =
    "https://github.com/SpyglassMC/vanilla-mcdoc/archive/6ef5413a6b0dcd4cbf448aedeebead491221c5cb.tar.gz";
...
fn render_nbt_schema_module(snapshot: &NbtSchemaSnapshot) -> String {
    ...
    output.push_str("        if let Some((_, node)) = entries.iter().find(|(entry_id, _)| *entry_id == id) {\n");
...
impl SchemaRenderer {
    ...
    fn render_node(&mut self, node: &SchemaNode) -> String {
        ...
        writeln!(self.definitions, "static {fields_name}: &[NbtSchemaField] = &[").unwrap();
